...
These values are commissioning parameters, not final research-tuned parameters. The obstacle-free formation, hold-zone hardware test, parameterized obstacle simulation, hardware obstacle dry run, and first physical obstacle-avoidance hardware baseline have now passed. The validated physical obstacle baseline uses a 15 cm obstacle, `d_obs_enter = 0.15 m`, and `d_obs_exit = 0.20 m`. The next stage is the mirrored repeatability and symmetry test with the obstacle placed closer to `robot2`.
...
For the first validated physical one-obstacle hardware baseline, use:
...
This simulation result was the evidence required before moving to hardware obstacle testing. The hardware dry run with `enable_motion:=false` and the first full physical obstacle-avoidance hardware baseline have now also passed. The next stage is the mirrored repeatability and symmetry test.
